...
export default {
  id: 'lock-screen',
  name: 'Lock screen',
  description: 'A quiet lock screen, with just 3.5 mm between the glass and its dusk wallpaper.',
  build({ THREE, w, h, size, glow, add, box, ring }) {
    ...
    // All UI stays behind the glass. Small depth gaps give a gentle parallax
    // without turning the familiar flat interface into floating blocks.
    ...
    add(new THREE.PlaneGeometry(w * 1.5, h * 1.5), wallpaper, 0, 0, -.0035);
    ...
    dune(-h * .12, -.0031, '#635066', .23);
    dune(-h * .26, -.0028, '#302d49', .32);
    dune(-h * .38, -.0025, '#22263e', .15);
    ...
    label('PARALLAX', -w * .43, h * .443, -.0005, size * .024, muted, 'left', w * .28);
    const bar = size * .009;
    for (let i = 0; i < 4; i++) {
      const height = size * (.016 + i * .009);
      box(w * .33 + i * bar * 1.5, h * .437 + height / 2, -.0005, bar, height, .00005, ink);
    }
    rounded(w * .423, h * .45, -.0005, size * .067, size * .03, size * .007, muted);
    rounded(w * .42, h * .45, -.00045, size * .049, size * .018, size * .003, ink);

    const clockX = landscape ? -w * .235 : 0;
    lock(clockX, h * .355, -.00075, size * .015);
    label('Friday, September 18', clockX, h * .284, -.001, size * .037, ink, 'center', w * .8);
    label('9:41', clockX, h * (landscape ? .12 : .19), -.0007, size * .235, ink, 'center', w * .8);
    label('18°  Clear skies', clockX, h * (landscape ? -.04 : .09), -.001, size * .029, muted);
    ...
    function notification(y, app, title, detail, time, color, symbol) {
      const z = -.0015;
      rounded(cardX, y - size * .006, z - .00018, cardWidth, cardHeight, size * .03, glow('#202238'));
    ...
    for (const x of [-shortcutX, shortcutX]) {
      add(new THREE.CircleGeometry(shortcutRadius, 40), glow('#454359'), x, shortcutY, -.0008);
    }
    rounded(-shortcutX, shortcutY - size * .005, -.0006, size * .012, size * .028, size * .004, ink);
    rounded(-shortcutX, shortcutY + size * .012, -.0006, size * .026, size * .011, size * .003, ink);
    rounded(shortcutX, shortcutY, -.0006, size * .042, size * .029, size * .006, ink);
    rounded(shortcutX - size * .008, shortcutY + size * .017, -.0006, size * .016, size * .008, size * .002, ink);
    ring(shortcutX, shortcutY, -.00053, size * .009, size * .0025, cardMaterial);
    label('Do Not Disturb', 0, shortcutY, -.0007, size * .024, muted, 'center', shortcutX * 1.45);
    rounded(0, -h * .466, -.00035, Math.min(w * .32, size * .5), size * .007, size * .0035, ink);
  },
};
